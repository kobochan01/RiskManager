...
| ヒヤリハット報告入力 | 発生日時・場所・クラス・園児名・けがの種類・事故内容の記録。発生時刻は07:00〜18:59の範囲をセレクトで選択。クラス・けがの種類はフリーワード追加可。園児名はオートコンプリート対応。タブ切り替えでも入力内容を保持。一括クリアボタン付き | 完了 |
...
| 機能 | 説明 | 状態 |
|------|------|------|
| 集計ダッシュボード | 時間帯別（1時間刻み、07:00〜18:59）・けが種類別・場所別のグラフ表示 + 期間フィルタ | 完了 |
| PDFレポート出力 | 集計グラフ・時間帯別集計表（午前/午後）・事案一覧をPDFファイルとして出力 | 完了 |
